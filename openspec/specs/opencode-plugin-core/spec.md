...
Defines the behavior of the OpenCode plugin at the core of OpenWolf. The plugin intercepts Read/Write/Edit tool calls and session lifecycle events to provide file anatomy lookups, learned-pattern checks against `cerebrum.md`, bug-log lookups against `buglog.json`, token tracking, and session-state surfacing through custom `wolf_status` and `wolf_search` tools. All plugin-originated messages visible to the agent MUST follow the trailing-delimited-block or system-prompt conventions and MUST NOT use `process.stderr.write()`, `process.stdout.write()`, leading prefixes on tool output, or emoji.
## Requirements
